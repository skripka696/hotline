...
class HotlineSpider(CrawlSpider):
    ...
    def parse_item(self, response):
        hxs = response
        item = HotlineItem()
        item['name'] = hxs.xpath('//*[@id="catalogue"]/ul/li/div/div/div/a/text()').extract()[0]
        print item['name']
        yield item
    ...
    def parse_description(self, response):
        ...
        item['description'] = hxs.xpath('/html/body/div/div/div/div/div/div/div/div/p/text()').extract()
        print type(hxs.xpath('/html/body/div/div/div/div/div/div/div/div/p/text()').extract()[0])
        yield item
